...
export class ServiceService {
  ...
  public getData() {
    return this.http.get<CurrencyFlag[]>(
      'https://currencyexchange-wbtr.onrender.com/images'
    );
  }

  public getrate(target: string) {
    return this.http.get<rate>(
      `https://currencyexchange-wbtr.onrender.com/rates?baseCode=USD&targets=${target}`
    );
  }
  private currenciesSubject = new BehaviorSubject<CurrencyFlag[]>([]);
  currencies$: Observable<CurrencyFlag[]> =
    this.currenciesSubject.asObservable();
